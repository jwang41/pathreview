...
class TestReviewService:
    ...
    async def test_list_reviews_ordered_by_created_at(self, mock_db_session):
        ...
        # list_reviews issues two queries: a count query and the paginated
        # query, so execute() is called twice, not once.
        assert mock_db_session.execute.call_count == 2

        # The paginated query (the second call) should order by created_at
        # descending.
        paginated_stmt = mock_db_session.execute.call_args_list[1].args[0]
        compiled = str(paginated_stmt)
        assert "ORDER BY" in compiled
        assert "reviews.created_at DESC" in compiled
